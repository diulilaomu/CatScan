...
class DataManager(
    ...
    private fun syncTemplateScansToResults() {
        ...
        if (activeTemplateId != null) {
            ...
            if (currentTemplate != null) {
                ...
                // id 用 scanData.id 的稳定 hash：同一条记录每次重建 id 不变，
                // 避免“同步后 id 重排导致点删除删错条”；冲突时递增避开
                val usedIds = HashSet<Long>()
                val scanResults = currentTemplate.scans.mapIndexed { index, scanData ->
                    var id = scanData.id.hashCode().toLong()
                    while (!usedIds.add(id)) id++
                    val uploaded = uploadedOf(scanData)
                    ScanResult(
                        id = id,
                        index = index + 1,
                        scanData = scanData.copy(uploaded = uploaded),
                        uploaded = uploaded
                    )
                }
    ...
    fun deleteTemplateScan(id: String, scanId: String) {
        // 先取要删除的记录内容：旧版扫描历史文件没保存 scanData.id，
        // 结果列表里的 id 与模板记录对不上时，按 模板+条码+时间戳 兜底匹配
        val targetScan = templates.firstOrNull { it.id == id }
            ?.scans?.firstOrNull { it.id == scanId }

        templateUseCases.deleteTemplateScan(id, scanId)

        // 同时从扫描结果中删除该数据
        val allScans = scanUseCases.getAllScans.invoke()
        val scanToDelete = allScans.find { result ->
            result.scanData.id == scanId ||
                (targetScan != null &&
                    result.scanData.templateId == id &&
                    result.scanData.text == targetScan.text &&
                    result.scanData.timestamp == targetScan.timestamp)
        }
    ...
    fun deleteScan(id: Long): ScanResult? {
        ...
        if (deleted != null && deleted.scanData.templateId.isNotBlank()) {
            ...
            if (idx != -1) {
                val template = templates[idx]
                // 旧版扫描历史文件没有保存 scanData.id，重载后 id 对不上模板里的记录，
                // 此时按 模板+条码+时间戳 兜底匹配同一条数据
                val updatedTemplate = template.copy(
                    scans = template.scans.filterNot { scan ->
                        scan.id == scanId ||
                            (scan.templateId == deleted.scanData.templateId &&
                                scan.text == deleted.scanData.text &&
                                scan.timestamp == deleted.scanData.timestamp)
                    }
                )
                templates.removeAt(idx)
                templates.add(idx, updatedTemplate)
    ...
    fun updateScan(id: Long, scanData: ScanData) {
        ...
        if (scanData.templateId.isNotBlank()) {
            ...
            if (template != null) {
                // 旧版扫描历史文件没保存 scanData.id，按 模板+时间戳 兜底匹配
                //（时间戳不会被编辑修改，内容可能已改所以不能按 text 匹配；只替换第一条，避免同毫秒记录被连带覆盖）
                val updatedScans = if (template.scans.any { it.id == scanData.id }) {
                    template.scans.map { if (it.id == scanData.id) scanData else it }
                } else {
                    val idx = template.scans.indexOfFirst {
                        it.templateId == scanData.templateId &&
                                it.timestamp == scanData.timestamp
                    }
                    if (idx >= 0) {
                        template.scans.toMutableList().also { it[idx] = scanData }
                    } else {
                        template.scans
                    }
                }
