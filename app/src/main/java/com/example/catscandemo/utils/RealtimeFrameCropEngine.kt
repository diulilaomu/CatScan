...
object RealtimeFrameCropEngine {
    ...
    private val lastErrorLogMs = AtomicLong(0)
    ...
    fun processNv21Frame(
        ...
        // elapsedRealtime 不受系统校时影响，做帧间隔节流更稳
        ...
        return try {
            ...
            // OpenCV so 加载失败/内存不足等都会走到这里；限频记录，通道2静默失效时留排查线索
            val nowMs = android.os.SystemClock.elapsedRealtime()
            if (nowMs - lastErrorLogMs.get() > 5_000L) {
                lastErrorLogMs.set(nowMs)
                android.util.Log.e("RealtimeFrameCropEngine", "processNv21Frame failed: ${e}")
            }
            null
        } finally {
            previewRgba?.release()
